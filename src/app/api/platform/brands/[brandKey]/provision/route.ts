...
export async function POST(req: NextRequest, ctx: { params: Promise<{ brandKey: string }> }) {
  ...
  const envLines = [
    "# Critical NEXT_PUBLIC vars so next build can inline them",
    `NEXT_PUBLIC_THIRDWEB_CLIENT_ID="${nextPublicThirdwebClientId}"`,
    `NEXT_PUBLIC_APP_URL="${nextPublicAppUrl}"`,
    `NEXT_PUBLIC_CHAIN_ID="${nextPublicChainId}"`,
    `NEXT_PUBLIC_BRAND_KEY="${nextPublicBrandKey}"`,
    `NEXT_PUBLIC_OWNER_WALLET="${nextPublicOwnerWallet}"`,
    `NEXT_PUBLIC_DEMO_MODE="${nextPublicDemoMode}"`,
    `NEXT_PUBLIC_RECIPIENT_ADDRESS="${nextPublicRecipientAddress}"`,
    `NEXT_PUBLIC_PLATFORM_WALLET="${nextPublicPlatformWallet}"`,
    `NEXT_PUBLIC_SPAWNCAMP_FACTORY_ADDRESS="${nextPublicSpawncampFactoryAddress}"`,
    `NEXT_PUBLIC_DISCORD_URL="${nextPublicDiscordUrl}"`,
    `NEXT_PUBLIC_GRAPHQL_URL="${nextPublicGraphqlUrl}"`,
    `NEXT_PUBLIC_BASE_USDC_ADDRESS="${nextPublicBaseUsdcAddress}"`,
    `NEXT_PUBLIC_BASE_USDC_DECIMALS="${nextPublicBaseUsdcDecimals}"`,
    `NEXT_PUBLIC_BASE_USDT_ADDRESS="${nextPublicBaseUsdtAddress}"`,
    `NEXT_PUBLIC_BASE_USDT_DECIMALS="${nextPublicBaseUsdtDecimals}"`,
    `NEXT_PUBLIC_BASE_CBBTC_ADDRESS="${nextPublicBaseCbbtcAddress}"`,
    `NEXT_PUBLIC_BASE_CBBTC_DECIMALS="${nextPublicBaseCbbtcDecimals}"`,
    `NEXT_PUBLIC_BASE_CBXRP_ADDRESS="${nextPublicBaseCbxrpAddress}"`,
    `NEXT_PUBLIC_BASE_CBXRP_DECIMALS="${nextPublicBaseCbxrpDecimals}"`,
    `NEXT_PUBLIC_BASE_SOL_ADDRESS="${nextPublicBaseSolAddress}"`,
    `NEXT_PUBLIC_BASE_SOL_DECIMALS="${nextPublicBaseSolDecimals}"`,
    `NEXT_PUBLIC_BEAR_CLOUD_API_URL="${nextPublicBearCloudApiUrl}"`,
    `NEXT_PUBLIC_BEAR_CLOUD_AUTH_URL="${nextPublicBearCloudAuthUrl}"`,
    `NEXT_PUBLIC_BEAR_CLOUD_API_KEY="${nextPublicBearCloudApiKey}"`,
    `NEXT_PUBLIC_BEAR_CLOUD_SECRET="${nextPublicBearCloudSecret}"`,
    `NEXT_PUBLIC_BEAR_CLOUD_SCOPE="${nextPublicBearCloudScope}"`,
    `NEXT_PUBLIC_DEMO_FREEZE_TIME="${nextPublicDemoFreezeTime}"`,
    `NEXT_PUBLIC_BEAR_CLOUD_TIMEOUT="${nextPublicBearCloudTimeout}"`,
    `NEXT_PUBLIC_ROBOTICS_ENABLED="${nextPublicRoboticsEnabled}"`,
    `NEXT_PUBLIC_ROBOTICS_USE_MOCK_FALLBACK="${nextPublicRoboticsUseMockFallback}"`,
    `NEXT_PUBLIC_THIRDWEB_ENGINE_WALLET="${nextPublicThirdwebEngineWallet}"`,
    `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY="${nextPublicStripePublishableKey}"`,
    "",
    "# Server-side vars loaded at runtime by Next.js",
    `CLOUDFLARE_API_TOKEN="${cloudflareApiToken}"`,
    `CLOUDFLARE_ZONE_ID="${cloudflareZoneId}"`,
    `CLOUDFLARE_ACCOUNT_ID="${cloudflareAccountId}"`,
    `THIRDWEB_SECRET_KEY="${serverThirdwebSecretKey}"`,
    `NODE_ENV="production"`,
    `STRIPE_API_KEY="${stripeApiKey}"`,
    `NEXT_PUBLIC_STRIPE_HEADLESS="${nextPublicStripeHeadless}"`
  ];
