  ...
  return (
    ...
    >
      ...
      {(challenge.walletAddress || challenge.network) && (
        ...
        >
          <span className="opacity-60 text-xs shrink-0">Transaction Ref:</span>
          <span className="font-mono font-semibold text-xs truncate max-w-[140px] sm:max-w-[220px]">
      ...
      <div className="space-y-1.5 text-left">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          ...
          <button
            ...
            className="text-xs font-semibold text-amber-400 hover:underline flex items-center gap-1 cursor-pointer shrink-0"
          >
            {copied ? (
              <>
        ...
        <div
          className={`p-3 rounded-xl font-mono text-xs leading-relaxed break-all select-all border max-h-28 overflow-y-auto ${
            isLightText
      ...
      {!livemode && (
        <div className="p-3.5 rounded-xl bg-indigo-500/10 border border-indigo-500/30 space-y-2 text-left animate-in fade-in">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <div className="flex items-center gap-1.5 text-indigo-300 text-sm font-bold">
            ...
            <button
              ...
              className="px-2.5 py-1 rounded-lg text-xs font-bold bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-300 border border-indigo-500/40 transition cursor-pointer shrink-0"
            >
              Auto-Fill "abcd"
            </button>
          </div>
      ...
      <div className="space-y-2 text-left">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          ...
          {typeof window !== "undefined" && (window as any).ethereum && (
            <button
              ...
              className="text-xs text-amber-400 hover:underline flex items-center gap-1 cursor-pointer disabled:opacity-50 shrink-0"
            >
              {signingWithWallet ? (
                <>
        ...
        <textarea
          ...
          className={`w-full p-3 rounded-xl font-mono text-sm focus:outline-none transition-all ${
            isLightText
      ...
      {(walletSignError || errorMessage) && (
        <div className="p-3.5 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400 text-sm flex items-start gap-2.5 animate-in fade-in text-left">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 pt-1">
        {onCancel && (
          <button
            ...
            onClick={onCancel}
            disabled={loading}
            className={`w-full sm:flex-1 py-2.5 rounded-xl text-sm font-bold border transition cursor-pointer disabled:opacity-50 ${
              isLightText
        ...
        <button
          ...
          className="w-full sm:flex-1 py-2.5 rounded-xl text-sm font-bold shadow-lg transition flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed hover:scale-[1.01] active:scale-[0.99]"
        ...
        >
          {loading ? (
            ...
            <>
              <KeyRound className="w-4 h-4" style={{ color: buttonTextColor }} />
