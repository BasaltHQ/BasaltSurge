...
const label = (value: string) => ({ ready: "Unsettled", session_mismatch: "Session mismatch", settled: "Settled", blocked: "Needs review", paid_settlement_pending: "Paid · sweep pending", needs_review: "Needs review", failed: "Failed", running: "Reconciling" }[value] || value);
...
export default function StripeAuditExplorer({ brands, onInspect }: { brands: string[]; onInspect: (receiptId: string) => void }) {
  ...
  async function scan(resume = false) {
    ...
    if (!resume) {
      ...
      setRows([]); setOutcomes({}); setScanned(0); setSelected(new Set()); setComplete(false); setPage(0);
  ...
  async function execute() {
    ...
    try {
      for (const row of queue) {
        ...
        setOutcomes(prev => ({ ...prev, [row.sessionId]: { status: "running", message: "Checking current Stripe and receipt state…" } }));
        try {
          const response = await fetch("/api/platform/stripe-audit", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "reconcile", sessionId: row.sessionId }) });
          const data = await response.json();
          if (!response.ok || !data.ok) throw new Error(data.error || "Reconciliation failed; refresh the audit before retrying.");
          if (data.row) setRows(prev => prev.map(r => r.sessionId === row.sessionId ? data.row : r));
          setOutcomes(prev => ({ ...prev, [row.sessionId]: { status: data.status || data.row?.finding || "needs_review", runId: data.runId, message: (data.details || []).map((d: any) => `${d.status}${d.reason ? `: ${d.reason}` : ""}`).join(" · ") || data.row?.reason || "Finished" } }));
        } catch (e) { setOutcomes(prev => ({ ...prev, [row.sessionId]: { status: "failed", message: e instanceof Error ? e.message : "Request failed; verify the receipt before retrying." } })); }
