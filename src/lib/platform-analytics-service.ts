...
const configCache = new Map<string, { rows: any[]; generatedAt: string; expiresAt: number }>();

function analyticsJson(body: any, status = 200) {
...
export async function loadAnalyticsResponse(req: NextRequest, partnerScope?: { brandKey: string }) {
  try {
    ...
    try {
      ...
      return analyticsJson({ ok: false, error: error instanceof Error ? error.message : "Invalid analytics query" }, 400);
    }
    ...
    try {
      ...
      if (cachedConfig && cachedConfig.expiresAt > Date.now()) {
        ...
        configCache.set(cacheScope, { rows: configs, generatedAt: configGeneratedAt, expiresAt: Date.now() + CACHE_TTL_MS });
      }
    ...
    const getReceiptBrandKey = (receipt: any) => partnerScope ? partnerScope.brandKey : resolveAnalyticsBrand(receipt, configMap);
    ...
    if (cached && cached.expiresAt > Date.now()) {
      ...
      if (collection) {
        ...
        projected = await collection.find(
          { type: "receipt", $or: [{ createdAt: dateFilter }, { createdAt: stringFilter }], ...(partnerScope ? partnerAnalyticsMongoBrandFilter(partnerScope.brandKey) : {}) },
        ...
        if (partnerScope) {
          const brandSql = partnerAnalyticsSqlBrandScope(partnerScope.brandKey);
          clauses.push(brandSql.clause); parameters.push(...brandSql.parameters);
        }
    ...
    if (collection && receipts.length) {
      const loadedDetails = await collection.find({ type: "receipt", _id: { $in: receipts.map(receipt => receipt._id) }, ...(partnerScope ? partnerAnalyticsMongoBrandFilter(partnerScope.brandKey) : {}) }, { readPreference: "secondaryPreferred" }).toArray();
      const detailRows = loadedDetails.filter((receipt: any) => !partnerScope || partnerAnalyticsRecordMatchesBrand(receipt, partnerScope.brandKey));
    ...
    // Legacy log IDs can collide across merchants. Partner evidence is loaded
    // through its separately authorized receipt/session-bound investigation API.
    if (!partnerScope && collection && pageReceiptIds.length && req.nextUrl.searchParams.get("includeLogPreview") !== "false") {
      try {
    ...
    const merchantStats = partnerScope && includeAggregates ? aggregateAnalyticsReceipts(allReceiptsLight.map(receipt => ({
      ...receipt,
      brandKey: String(receipt.merchantWallet || receipt.wallet || "unknown").trim().toLowerCase(),
      brandName: receipt.merchantName || receipt.shopName || receipt.shopSlug || String(receipt.merchantWallet || receipt.wallet || "Unknown merchant"),
    })), scope.timeZone).brandStats : undefined;
    ...
    return analyticsJson({
      ...
      ...(partnerScope ? { merchantStats: merchantStats || [] } : {}),
      recentReceipts: processedReceipts,
      dailySeries: aggregates.dailySeries,
      comparison,
      metadata: {
        definitionVersion: ANALYTICS_DEFINITION_VERSION,
        ...(partnerScope ? { accessScope: { type: "partner", brandKey: partnerScope.brandKey, attribution: "explicit-brand-only", legacyUnattributedReceipts: "excluded" } } : {}),
        generatedAt,
    ...
    return analyticsJson({ ok: false, error: status === 500 ? "Analytics could not be loaded. Please retry." : error.message }, status);
  }
}
