...
async function run() {
    ...
    try {
        ...
        if (!doc) {
            throw new Error(`Brand config document not found for key: ${key}`);
        }
        console.log(`Fetched config for ${key}: ${doc.name}`);
        await client.close();
        ...
        // Build the echo statements. Escape single quotes inside single quotes: ' -> '\''
        const echoCommands = [
        ...
        console.log("Updating Git deployment actions on Plesk to write .env.production using echo chain...");
        const branch = process.env.PLESK_GIT_BRANCH || "main";
        
        // Single-line command actions chain
        const gitActions = `export PATH=/opt/plesk/node/24/bin:$PATH && cd /var/www/vhosts/basalthq.com/${cleanDomain} && ${writeEnvCommand} && echo "=== Verification ===" && ls -l .env.production && head -n 10 .env.production && echo "====================" && set -a && . .env.production && set +a && npm install && npm run build && mkdir -p tmp && touch tmp/restart.txt`;
        ...
        console.log("Triggering Git deploy (pull/deploy actions) to execute the write and build...");
