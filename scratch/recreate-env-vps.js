...
async function run() {
    ...
    try {
        ...
        // Build the environment dictionary exactly like route.ts does
        const baseEnv = {};
        const allowPrefixes = [
        ...
        const envLines = sortedEnv.map(([k, v]) => `${k}="${String(v || "").replace(/"/g, '\\"')}"`);
        const envContent = envLines.join("\n");

        console.log("Updating Git deployment actions on Plesk to include env creation heredoc...");
        const branch = process.env.PLESK_GIT_BRANCH || "main";
        
        // Heredoc to write .env.production securely before doing build
        const gitActions = `export PATH=/opt/plesk/node/24/bin:$PATH && cd /var/www/vhosts/basalthq.com/${cleanDomain} && cat << 'EOF' > .env.production\n${envContent}\nEOF\n && set -a && . .env.production && set +a && npm install && npm run build && mkdir -p tmp && touch tmp/restart.txt`;
        ...
        console.log("Triggering Git deploy (pull/deploy actions) to write the file and build...");
